...
# but ig_cookies.json is a Playwright storage_state JSON. Passing the JSON
# straight to yt-dlp silently does nothing, so downloads go out unauthenticated
# and Instagram replies "rate-limit reached or login required". We convert the
# JSON to a real Netscape file (once, cached) and hand THAT to yt-dlp.

_NETSCAPE_COOKIES = None  # cached temp path for this process


def _json_cookies_to_netscape(json_path, out_path):
    ...
    lines = ["# Netscape HTTP Cookie File",
             f"# Generated by ig_collector.py from {json_path}", ""]
    written = 0
    for c in cookies:
        name = c.get("name")
        domain = c.get("domain")
...
def netscape_cookies(json_cookie_file):
    """Return a Netscape cookies.txt path for yt-dlp, or None if unavailable.

    Converts once per process and caches the temp file (cleaned up on exit;
    it holds session secrets so it must not linger).
    """
    global _NETSCAPE_COOKIES
    if _NETSCAPE_COOKIES is not None:
        return _NETSCAPE_COOKIES
    if not Path(json_cookie_file).exists():
        return None
    fd, tmp = tempfile.mkstemp(prefix="yt_dlp_cookies_", suffix=".txt")
    os.close(fd)
    if _json_cookies_to_netscape(json_cookie_file, tmp):
        _NETSCAPE_COOKIES = tmp
...
def download_video(url, code, browser_cookies_file):
    if not Path(browser_cookies_file).exists():
        ...
        return False
    cookies = netscape_cookies(browser_cookies_file)
    if not cookies:
    ...
    async with async_playwright() as p:
        ...
        for i, url in enumerate(urls, 1):
            ...
            if args.download and ok:
                dest = video_Dir / f"{code}.mp4"
                cookies = netscape_cookies(cookie_File)
                if not cookies:
